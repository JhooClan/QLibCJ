...
def factor2(num): # Algoritmo Las Vegas que factoriza un numero N = p*q, p y q primos
    ...
    print ("choosen a = ", a)
    p = reduce(gcd, (a, num)) # Compruebo si son coprimos. Si el resultado es distinto a 1 habremos obtenido p
    if (p != 1):
    ...
    for r in range(1, num):
        if pow(a, r, num) == 1: # Si se cumple que a^r = 1 (mod p*q)
            ...
            if r % 2 == 0: # Comprobamos si r es par
                print ("r is even")
                x = pow(a, int(r/2))
                if (x + 1) % num != 0: # Comprobamos si x + 1 != 0 (mod p*q)
                    p = reduce(gcd, (x - 1, num))
                    q = int(num/p)
                    print ("Time elapsed: ", (time.process_time() - start), " seconds")
                    return (p, q)
                else:
